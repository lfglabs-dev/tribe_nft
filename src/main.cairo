...
@storage_var
func _blacklisted(quest_id, task_id, user_id) -> (is_blacklisted: felt) {
}
...
func mint{
...
}(tokenId: Uint256, sig: (felt, felt), quest_id, task_id, user_id) {
    alloc_locals;
    ...
    let (hashed) = hash2{hash_ptr=pedersen_ptr}(hashed, user_id);
    ...
    // check if this reward has been blacklisted
    let (is_blacklisted) = _blacklisted.read(quest_id, task_id, user_id);
    assert is_blacklisted = FALSE;

    // blacklist that reward
    ERC721._mint(caller, tokenId);
    _blacklisted.write(quest_id, task_id, user_id, TRUE);
